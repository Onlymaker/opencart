...
?>
	<script  type = "text/javascript" ><!--
		...
		poip_list_custom.prototype.showOneProductImages = function(elem_image, poip_data) {
			...
			// this theme does not have a link on for the image
			//var current_a_elem = elem_image.parent(); 
			var current_class_image_id = elem_image.attr('data-poip_id');
			var product_id = this_object.getProductIdFromPOIPId(current_class_image_id);
			var product_href = '';
			var product_link_elem = elem_image.closest('product-item-container').find('a[href]:not(.quickview)');
			if ( product_link_elem.length ) {
				product_href = product_link_elem.attr('href');
			}
